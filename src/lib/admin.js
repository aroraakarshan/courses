...
if(t==='availability'){
const a=await fetch('/api/availability').then(r=>r.json())
...
if(t) t.innerHTML='<tr><th>Code</th><th>Discount</th><th>Service</th><th>Used/Max</th><th></th></tr>'+r.map(c=>'<tr><td>'+c.code+'</td><td>'+c.discount_percent+'%</td><td>'+(c.service||'All')+'</td><td>'+c.used+'/'+(c.max_uses||'∞')+'</td><td><button class="btn btn-d" onclick="delCp(\''+c.code+'\')">Deactivate</button></td></tr>').join('')
}
