...
import { c as createComponent, r as renderTemplate, a as createAstro } from '../chunks/astro/server_C0dJqwLY.mjs';
export { renderers } from '../renderers.mjs';

const ADMIN_HTML = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Admin — Courses</title><style>
...
const $$Admin = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (Astro2.request.method === "POST") {
    ...
    const secret = Astro2.locals.runtime.env.ADMIN_PASSWORD;
    if (secret && pw === secret) showDashboard = true;
    else error = true;
  }
  if (!showDashboard) {
    return new Response(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Admin Login</title><style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:Inter,system-ui,sans-serif;background:#fafaf7;display:flex;align-items:center;justify-content:center;min-height:100vh}
    .box{background:#fff;border:1px solid #e5e5e0;border-radius:12px;padding:32px;width:340px;text-align:center}
    h2{margin-bottom:1.5rem;font-weight:700;letter-spacing:-0.02em}
    input{width:100%;padding:.7rem;border:1px solid #e5e5e0;border-radius:6px;font-size:.85rem;font-family:inherit;margin-bottom:12px}
    input:focus{outline:none;border-color:#C45D2C}
    button{width:100%;padding:.7rem;background:#C45D2C;color:#fff;border:none;border-radius:6px;font-size:.9rem;font-weight:600;cursor:pointer}
    ${error ? ".err{color:#9A3324;font-size:.8rem;margin-top:.5rem}" : ".err{display:none}"}
  </style></head><body><div class="box">
    <h2>Admin Login</h2>
    <form method="POST">
      <input type="password" name="password" placeholder="Password" autofocus/>
      <button type="submit">Login</button>
      ${error ? '<p class="err">Invalid password</p>' : '<p class="err"></p>'}
    </form>
  </div></body></html>`, { headers: { "Content-Type": "text/html" } });
  }
  return renderTemplate`${ADMIN_HTML}`;
}, "/Users/akarshanarora/Developers/courses/src/pages/admin.astro", void 0);

const $$file = "/Users/akarshanarora/Developers/courses/src/pages/admin.astro";
const $$url = "/admin";
